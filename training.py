...
def f1_score_new(test_labels, predictions):
    ...
    smp_report = precision_recall_fscore_support(
        np.array(P), np.array(T), average='samples')
    del P
    del T
    del r
    return x, smp_report
...
def get_best_classifier(train_bags, train_labels, test_bags, test_labels):
    ...
    for C in C_2d_range:
        for gamma in gamma_2d_range:
            ...
            classifiers['miSVM_' + str(C)] = misvm.NSK(kernel='linear_fs',
                                                       gamma=gamma,
                                                       C=C)
# *********************************** Training Phase ******************************************** #
    ...
    for algorithm, classifier in classifiers.items():
        ...
        cut_off[algorithm], accuracies[algorithm], PREDICTOIN[
            algorithm], REPORT[algorithm] = chosse_threshold(
                predictions,
                test_labels)  #calculate_accuracy() #np.average(test_labels == np.sign(predictions))
...
if __name__ == '__main__':
    ...
    for j in range(1):
        ...
        for i in range(TOTAL_LABEL):
            ...
            classifier, cut_off, max_accuracy = get_best_classifier(
                train_bags, label_train, test_bags, label_test)
